...
// Real model IDs and pricing, sourced directly from each provider's own
// current docs (OpenAI: developers.openai.com/api/docs/models; Google:
// ai.google.dev/gemini-api/docs/pricing — both checked live, Aug 2026).
// Anthropic doesn't publish its current per-token pricing anywhere this app
// can verify automatically, so its tiers are labeled by relative cost only,
// not a fabricated number — check console.anthropic.com for the real rate.
// gpt-5.6-luna and gemini-3.6-flash are already this app's defaults; Luna
// is already OpenAI's own cheapest general-purpose model, so nothing
// changes there — Gemini's default (3.6-flash) is NOT its cheapest, and
// Anthropic's default (sonnet-5) is a deliberate mid-tier choice, not cheap.
interface ModelTierOption { value:string; tier:string; price:string }
const MODEL_TIERS:Record<AIProvider,ModelTierOption[]>={
  openai:[
    {value:'gpt-5.6-luna',tier:'Cheapest',price:'$0.20 / $1.20 per 1M tok'},
    {value:'gpt-5.6-terra',tier:'Balanced',price:'$2.00 / $12.00 per 1M tok'},
    {value:'gpt-5.6-sol',tier:'Flagship',price:'$4.00 / $20.00 per 1M tok'},
  ],
  anthropic:[
    {value:'claude-haiku-4-5-20251001',tier:'Cheapest',price:'lowest-cost current Claude model'},
    {value:'claude-sonnet-5',tier:'Balanced',price:'this app\'s default'},
    {value:'claude-opus-5',tier:'Flagship',price:'most capable current Claude model'},
  ],
  gemini:[
    {value:'gemini-2.5-flash-lite',tier:'Cheapest',price:'$0.10 / $0.40 per 1M tok'},
    {value:'gemini-3.6-flash',tier:'Balanced',price:'$0.75 / $3.75 per 1M tok'},
    {value:'gemini-3.1-pro-preview',tier:'Flagship',price:'$2.00 / $12.00 per 1M tok'},
  ],
};
...
const BEHAVIOR_CHANGELOG:Array<{version:string;summary:string}> = [
  {version:'3.26.1',summary:"Three real bugs, all traced from one live transcript Robbie sent rather than guessed at. (1) \"Is the back door locked?\" got voice-transcribed to \"There's the back door locked?\" — no bare \"is\"/\"are\" token at all — so it missed the status-question pattern entirely and Axiom answered \"Yes, locked\" from nothing, only correcting itself to the real (unlocked) state after being challenged. Added the exact contraction this real garble needs, deliberately not widened further (was/were/has/does would false-trigger on ordinary sentences containing \"on\"/\"off\"). (2) The first \"Lock the back door\" got an invented, non-standard \"reply the same phrase to approve\" response instead of ever calling the real tool — only an identical second request actually triggered the genuine AX-XXXXXX approval flow. Root cause: action requests offered both homebridge_snapshot and homebridge_control together, diluting the pool to 2 candidates — Axiom's hard exact-name tool forcing (the one mechanism proven reliable across all three providers) only ever engages for a sole candidate; with 2, it fell back to the softer \"a tool is required\" hint, which has a known history of being ignorable. An action request now resolves to homebridge_control alone, restoring the guarantee — nothing is lost, since the control tool already re-reads and verifies the accessory itself after the change. (3) The approval preview shown for closing an application window read a field (`args.target`) that doesn't exist on that tool's real schema (`args.application`) — the actual app name was silently blank in every single one of these approvals, shown as \"Close application window .\", asking Robbie to approve an action with no way to tell what it would actually do. Now reads the real field and shows the real name. Real tests added for all three, including the exact garbled phrase from the transcript. Full build and test suite clean."},
  {version:'3.26.0',summary:"Robbie's ask: a real guided first-run onboarding — the app's entire prior first-run experience was one line (\"if no AI key, force-open the dense 10-section Settings modal\"), no explanation of what's required versus optional. Designed and shown as a standalone interactive mockup first, approved before any real code, per this project's standing rule for UI work. Built as a new full-screen sequence (Onboarding.tsx + onboarding.css): a boot-log intro, then six steps — identity (optional, saves a name via a real person-kind memory in the exact phrasing the app's own identity parser already expects), AI provider + key (the one genuinely required step, with a live TEST UPLINK that saves the key for real and calls the real provider-test endpoint before letting you continue — not a fake success animation), voice, presence/consent (plain-language explanation before the real acknowledgeBiometricConsent flag), Homebridge, and appearance, ending in a system-online summary. Every step reuses the app's own real saveSettings/testProvider/addMemory/saveConnector calls — nothing is a separate parallel data path. Added a genuine escape hatch (EXIT TO SETTINGS, available on every step) after realizing the new wizard was stricter than the app's own prior first-run behavior, which always let you close Settings without a key; a new hasCompletedOnboarding flag tracks only whether the guided wizard itself was finished, kept deliberately separate from hasSelectedAIKey so exiting early to configure a key the plain way still counts as done and never traps a user in the wizard on every later launch. Migration handled explicitly: any existing install's data file predates this field entirely, and is treated as already onboarded rather than replayed through the wizard — verified with a real test that writes a file missing the field and confirms it loads as complete. Full build, typecheck, and test suite clean, including new store-level migration tests. Honest limitation: this sandbox cannot launch Axiom's real Electron GUI, so the wizard's actual on-screen behavior against the live app is unverified from here — the CSS and layout are a structural port of the already-approved, browser-tested mockup, but the real first real look is Robbie's."},
  {version:'3.25.0',summary:"Robbie asked if he could set his own approval phrase instead of the per-action AX-XXXXXX code — flagged the real trade-off first (a static phrase can't tell two simultaneously-pending actions apart the way a fresh random code can) and let him choose knowing that; he chose the static phrase anyway for the convenience. Built it as a genuinely separate secret from the existing owner override phrase (different purpose — this approves a blocked action, that restores identity trust when biometrics fail — so they're independently settable and clearable, never sharing storage). Same security discipline as that existing phrase: scrypt-hashed, salted, rate-limited to 5 attempts before a 10-minute lockout, never sent to any AI provider, and only even attempted when something is genuinely awaiting approval so ordinary conversation never pays the scrypt cost or risks a false trigger. When exactly one action is pending, saying the phrase resolves it directly, same as typing the exact code; with more than one pending, Axiom refuses to guess and asks for the real code or CORE instead, rather than silently picking one — the exact edge case Robbie was warned about going in. New Settings field alongside the existing owner override phrase, same visual pattern. Real tests added at the store layer (enrollment, exact-match verification, lockout, hashed-not-plaintext storage, clearing, and independence from the owner override phrase) — the request-routing branch itself lives in main.ts, which stays outside this project's existing unit-test boundary, consistent with how the AX-XXXXXX code path already worked before this."},
  {version:'3.24.4',summary:"Follow-up to 3.24.3's Homebridge empty-cache finding: Robbie's ask was direct — don't just explain the quirk, have Axiom fix it itself, invisibly. Built a real, hidden, one-shot warmup: when Homebridge reports zero accessories, Axiom now opens a hidden background window, logs into Homebridge's own web UI using the real login form (sourced directly from its actual login.component.html — #form-username, #form-pass, #submit-button, not guessed selectors) with the same username/password already saved for the API, loads its Accessories tab once, then retries the real API call. This is a separate, dedicated hidden window from the user-facing Axiom Browser (which is HTTPS-only by design and always shows itself) — Homebridge is typically plain HTTP on the local network, and this must never be visible or steal focus. If the automatic warmup doesn't help (or fails outright — network hiccup, changed login form, etc.), Axiom says so honestly rather than silently retrying forever or claiming success it can't back up. Real tests added exercising the actual hidden-window flow end to end (login field-fill, form submission, the Accessories-tab navigation, and the retry), not just checking that some error text got returned — caught and fixed a real mock bug in the process (BrowserWindow's own .loadURL() is a different method from .webContents.loadURL(), and the initial test only stubbed the second one)."},
  {version:'3.24.3',summary:"Robbie's Homebridge dashboard showed dozens of real accessories (locks, sensors, cameras, alarm), but Axiom reported seeing none — signed in, but blind. Root-caused live rather than guessed, and it turned out not to be an Axiom bug at all: a documented Homebridge Config UI X quirk (github.com/homebridge/homebridge-config-ui-x issue #1005) — after a restart, the same GET /api/accessories endpoint Axiom calls genuinely returns an empty array until someone opens the Accessories tab in Homebridge's own web UI at least once; that page load is what populates Config UI X's internal accessory cache, not the plain REST call. Confirmed exactly against Robbie's real instance: Axiom saw nothing, then correctly listed every device the moment the Homebridge dashboard had been opened in a browser. Since this isn't fixable from Axiom's side (it's the upstream server's own cache, not a request Axiom is getting wrong), homebridgeSnapshot() now recognizes a genuinely empty response and attaches real guidance instead of just looking broken — connection stays reported as healthy, since the login and request both legitimately succeeded. Also fixed a related renderer bug caught while making this change: the Smart Home panel treated the mere presence of any hint text as \"CONNECTION DEGRADED\" (a red fault state), which would have made this exact benign, connected, guidance-only case look like a real outage — now keyed strictly off the actual connection flag, with the hint shown as calm guidance instead of an alarm."},
  {version:'3.24.2',summary:"Traced the exact live transcript Robbie sent from 3.24.0, turn by turn, rather than guessing at the ADAPTIVE FAILOVER clustering he'd flagged as a suspect. It turned out not to be the real cause: every one of Axiom's confusing or self-contradicting replies in that conversation lines up exactly with which turns did or didn't offer a gods-eye tool at all, traced against the actual trigger regexes. \"I don't see it\" and \"there's no map or computer-control tool available here\" both landed on turns where literally zero gods-eye tools were offered — the model wasn't lying or corrupted, it genuinely had nothing to call, because \"You gotta fly the map to St. Louis, Missouri\" said \"map,\" not \"globe,\" had no latitude/longitude wording, and named no live-data domain word, so it matched none of the existing trigger patterns — the exact same keyword-gating blind spot this project has hit and fixed many times before (web search, folder creation, Homebridge, and twice already this week for this same feature). Added a bare-verb pattern (fly/move/navigate/pan/zoom + map/camera/globe/view) to close it. Separately, while tracing this, found a real related gap worth closing on its own: mayMutate() — the check that disables cross-provider failover for anything that might replay a real side effect against a second provider — didn't recognize \"fly\"/\"zoom\"/\"pan\"/\"navigate\" at all, so a God's Eye View request built from exactly those words (\"zoom in on Tokyo\") sailed through with full automatic failover enabled despite being a genuinely mutating action (a real page reload against the embedded globe). Added those four verbs; deliberately left out \"find\"/\"track\"/\"show me\" as too common in ordinary non-mutating conversation to be worth losing failover resilience over. Real tests added for the new trigger. Honest about what's still open: whether the ADAPTIVE FAILOVER clustering in that transcript was pure coincidence (OpenAI having a rough few minutes) or was actually contributing something the turn-by-turn trace doesn't show — that would need Robbie's real diagnostics log to settle, not another inference from the visible transcript alone."},
  {version:'3.24.1',summary:"Robbie tested 3.24.0 live and the camera genuinely never moved, despite Axiom's own event trace showing a verified GODS EYE FLY TO — the screenshot he sent proved it: the visible view was still sitting over the app's own hardcoded default location (Austin, TX), never St. Louis. Root-caused by reading the real app's source directly rather than guessing again: src/sharelink.js parses location.hash exactly once, in parseInitialHash(), called only at initial page mount — there is no hashchange listener anywhere in it. 3.24.0's fix wrote a real, correct hash and got a real, honest success response from the browser (no exception, hence 'verified') — but nothing in the running page was listening, so it was silently inert. This is a genuine architecture correction, not a tweak: every flyTo()/setLayers() call now writes the hash and then forces a real reload() — confirmed to be the one mechanism that actually re-runs the app's hash-driven startup logic against the new value. Found and fixed a second real bug while tracing this: open_gods_eye_view re-opening an already-positioned view compared its URL to the bare default with strict equality, which now always differs once a real flyTo() has run (the view's URL legitimately carries a #hash) — that mismatch was silently reloading the view back to its bare default, wiping the camera position and every enabled layer, every time. Real tests added asserting the hash-write happens before the reload (not after — order matters) and that a positioned, already-open view survives being reopened. Separately, unresolved and flagged rather than guessed at: Axiom's own text replies across that conversation were inconsistent and at one point flatly denied having any map or computer-control tool at all, which is false — the event trace shows three ADAPTIVE FAILOVER (OpenAI) recoveries clustered in the same short window, suggesting a provider reliability issue may have been corrupting what the model actually knew about its own prior tool calls; real diagnosis of that needs Robbie's actual runtime log, not another guess from here."},
  {version:'3.24.0',summary:"Robbie's real, live failure: with God's Eye View already open, he asked Axiom to \"find me live flights over St. Louis, MO\" and Axiom did nothing at all. Two separate root causes, both fixed. (1) The keyword trigger for gods_eye_fly_to required the literal phrase \"God's Eye View\" in every message — Robbie's request never said it, since the panel was already open and obviously the context, so zero gods-eye tools were even offered; the model had nothing to call. Broadened the trigger to fire on the live-data domain words themselves (flights, aircraft, ships, satellites, earthquakes, military installations, and the rest of the app's real layer set) combined with a live/tracking/show-me verb, without requiring the app's name every single turn. (2) Even with the tool offered, there was no way to ask for it — Axiom could only move the camera, never turn on the actual flights layer that makes aircraft visible, so \"find flights over St. Louis\" could at best have flown the camera to an empty patch of sky. Read the real God's Eye View source directly off GitHub (src/data/layerState.js) rather than guessing: found its real 16-layer registry (flights, ais-live-vessels, satellites, earthquakes, cctv, military, traffic, and more) and the exact single-character URL tokens it encodes them with. Added a real setLayers() capability using those confirmed tokens, exposed as a new gods_eye_set_layers tool and as an optional `layers` argument directly on gods_eye_fly_to, so \"find flights over St. Louis\" now flies the camera there AND turns on the flights layer in one call. Also fixed a real latent bug found while doing this: the old flyTo() overwrote the entire URL hash on every call, which would have silently turned off any layer a separate setLayers() call had just enabled — camera and layer writes now merge into whatever's already on the hash instead of replacing it outright, confirmed against the real app's own hash format (independent query keys per aspect, not one monolithic string). Both tools now open the view automatically if it isn't already open, so a single request can open-fly-and-enable-layers without a separate turn. Real unit tests added for the layer registry, the merge-vs-overwrite behavior, and the broadened trigger. Full build, typecheck, and test suite clean. Still genuinely unverified: whether God's Eye View's live UI actually reacts to these exact hash keys the way its source suggests — that's the real next test, on Robbie's machine, with real flights actually appearing over St. Louis."},
  {version:'3.23.2',summary:"Robbie's ask: a real way to pick a model per AI service, favoring the cheapest tier for his use — the plumbing already existed (Axiom already remembers a separate model per provider and reloads it when you switch provider tabs in Settings) but the UI was one freeform text field with a single combined suggestion list and no indication of which model was actually cheap. Rather than guess at model names, looked them up live against each provider's own current documentation before writing anything down: OpenAI's own docs (developers.openai.com/api/docs/models, fetched live) confirm gpt-5.6-luna — this app's existing OpenAI default — is already its cheapest general-purpose model, so nothing changes there. Google's own pricing docs (ai.google.dev/gemini-api/docs/pricing, fetched live) show gemini-2.5-flash-lite at $0.10/$0.40 per 1M tokens is meaningfully cheaper than this app's default gemini-3.6-flash ($0.75/$3.75) — that one's a real, actionable switch. Anthropic doesn't publish current per-token pricing anywhere this app can verify automatically, but claude-haiku-4-5-20251001 is confirmed the current lineup's cheapest by model family alone, labeled as such without a fabricated price. Added a real per-provider tier picker (Cheapest/Balanced/Flagship, one click each) above the existing freeform model field — the free-text field and its autocomplete stay, now scoped to the selected provider's own three tiers, since a provider can always ship a new model this app doesn't know about yet and typing an exact ID must keep working. Declined to invent additional OpenAI/Gemini tier names (a web search surfaced several SEO-aggregator pricing pages with plausible-looking but unverifiable model names) — used only what each provider's own primary documentation actually confirmed. Not yet visually verified running inside the real Electron app (this sandbox can't launch its GUI); typecheck, full build, and the existing test suite are clean, and the new markup reuses the exact same button/grid patterns already proven elsewhere in Settings."},
  {version:'3.23.1',summary:'Robbie hit a real, live failure twice in a row: "forecast" and "weather 63049" both came back BLOCKED with "Live Google search returned no parseable results," no way to tell why. Root-caused rather than guessed: web_search has no dedicated weather path — both queries were routed through the same Google-scraping tool (googleSearch.ts) that only ever looks for h3-linked result links, but Google answers a weather query entirely inside its own inline widget instead, with zero h3 links on the page at all. That widget\'s element ids (wob_wc/wob_tm/wob_dc/wob_loc/wob_hm/wob_ws/wob_pp) have been stable for years independent of the rest of the page\'s markup churn, the same reasoning already used for the h3 walk — added real extraction for it, returned as a normal search result (temperature, condition, location, humidity, wind, precipitation) so it flows through the existing pipeline unchanged. Separately fixed the actual diagnostic gap this bug exposed: a genuine "no results" and Google\'s own automated-traffic/CAPTCHA block page failed identically before, with no way to tell which happened without opening the hidden window by hand. The page\'s own title and a short text sample are now captured on every extraction pass; when nothing is found, the error names the real page Google showed and specifically calls out Google\'s own automation-detection wording if that\'s what it was, instead of a generic guess. Could not verify either fix against live google.com directly — this sandboxed environment has google.com browsing blocked outright — so the weather-widget element ids are built from well-documented, long-stable public knowledge of that widget rather than a fresh live check; that live verification is the real next step, same honesty boundary as the God\'s Eye View work below. All existing and new googleSearch tests pass; full build and typecheck clean. On the separate, much broader ask to "deep search every line of code and fix every issue" — that\'s not something to attempt honestly in one unscoped pass on a codebase this size, especially one that\'s already been through several full audits this project (v3.6.1, v3.20.0); flagged rather than faked, happy to run a scoped audit if Robbie wants one.'},
...
export default function App() {
  ...
  const [actionApprovalDraft,setActionApprovalDraft]=useState('');
  ...
  useEffect(() => {
    Promise.all([window.axiom.getSettings(), window.axiom.loadHistory(), window.axiom.listMemories(), window.axiom.listGoals(), window.axiom.listPermissions(),window.axiom.loadAudit(), window.axiom.getRuntimeSnapshot(),window.axiom.getDesktopGraph(),window.axiom.getAppInfo()]).then(([nextSettings, history, savedMemories, savedGoals, permissionList,auditItems,runtimeSnapshot,graphSnapshot,appInfo]) => {
      ...
      // hasCompletedOnboarding tracks only whether the guided wizard was
      // ever finished, not whether Axiom is actually configured — someone
      // who used the wizard's own "exit to Settings" escape hatch (or an
      // upgraded install predating this field, handled by store.ts's own
      // migration) may already have a working key without ever finishing
      // it, and must not be forced back into the wizard on every launch
      // just because they didn't click through to the end.
      if (!nextSettings.hasCompletedOnboarding && !nextSettings.hasSelectedAIKey) setShowOnboarding(true);
      else if (!nextSettings.hasSelectedAIKey) setSettingsOpen(true);
  ...
  const clearActionApproval=async()=>{try{const next=await window.axiom.saveSettings({provider:providerDraft,model:modelDraft,codingWorkspace:workspaceDraft,clearActionApprovalPhrase:true});setSettings(next);setSettingsStatus('Approval phrase removed. AX-XXXXXX codes are the only way to approve actions now.');}catch(reason){setSettingsStatus(reason instanceof Error?reason.message:String(reason));}};
  ...
  return <main className={`shell mode-${visual.mode} theme-${appearance.color} motion-${effectiveMotion} density-${appearance.density}`} style={shellStyle}>
    {showOnboarding&&<Onboarding onComplete={async(next)=>{setSettings(next);setAppearance(normalizeRendererAppearance(next.appearance));setMemories(await window.axiom.listMemories());setShowOnboarding(false);if(!next.hasSelectedAIKey)setSettingsOpen(true);}} onExit={()=>{setShowOnboarding(false);setSettingsOpen(true);}}/>}
      ...
      <section className="settings-card">
        ...
        <div className="settings-scroll">
          ...
          <div className="settings-grid provider-keys">
            ...
            <label>Approval phrase <em>{settings?.hasActionApprovalPhrase?'SAVED':'EMPTY'}</em><input type="password" value={actionApprovalDraft} onChange={(event)=>setActionApprovalDraft(event.target.value)} placeholder={settings?.hasActionApprovalPhrase?'Encrypted — enter to replace':'A secret phrase, 8+ characters'} autoComplete="off"/>{settings?.hasActionApprovalPhrase&&<button className="key-clear" onClick={()=>void clearActionApproval()}>REMOVE PHRASE</button>}<span className="settings-hint">Optional shortcut for approving a blocked action: say or type this exact phrase instead of the AX-XXXXXX code. Only works when exactly one action is awaiting approval — with more than one pending, use the exact code or CORE instead, since a static phrase can't tell them apart. Locks out for 10 minutes after 5 wrong attempts.</span></label>
          </div>
          <div className="model-tier-picker">{MODEL_TIERS[providerDraft].map((option)=><button key={option.value} type="button" className={modelDraft===option.value?'active':''} onClick={()=>setModelDraft(option.value)}><b>{option.tier.toUpperCase()}</b><span>{option.value}</span><em>{option.price}</em></button>)}</div>
          <div className="settings-grid compact"><label>Active model <em>PER-PROVIDER — SWITCHING TABS ABOVE REMEMBERS EACH ONE</em><input list="model-options" value={modelDraft} onChange={(event)=>setModelDraft(event.target.value)} placeholder="Or type any exact model ID"/><datalist id="model-options">{MODEL_TIERS[providerDraft].map((option)=><option key={option.value} value={option.value}/>)}</datalist></label><button className="secondary" onClick={()=>void (async()=>{await saveSettings();await testProvider(providerDraft);})()}>TEST AI CONNECTION</button></div>
