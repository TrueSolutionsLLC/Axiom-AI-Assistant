...
export class GodsEyeViewManager {
  ...
  async flyTo(target: GodsEyeFlyTo): Promise<void> {
    ...
    const hash = `v=2&lat=${target.lat}&lon=${target.lon}&alt=${target.alt ?? 2000}&heading=${target.heading ?? 0}&pitch=${target.pitch ?? -30}&roll=0&style=normal&map=photoreal&hud=tactical`;
    await this.view.webContents.executeJavaScript(`location.hash = ${JSON.stringify(hash)};`);
  }
