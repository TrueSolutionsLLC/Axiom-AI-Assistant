...
export class ConnectorClient {
  ...
  async homebridgeSnapshot():Promise<HomebridgeSnapshot>{const status=this.statuses().find((item)=>item.id==='homebridge')!,generatedAt=new Date().toISOString();if(!status.configured)return{configured:false,connected:false,endpoint:status.endpoint,generatedAt,accessories:[],counts:{},error:'Homebridge is not configured.'};try{const raw=await this.homebridgeJson('/api/accessories') as unknown[],accessories=(Array.isArray(raw)?raw:[]).map((item)=>this.normalizeHomebridgeAccessory(item)).filter((item):item is HomebridgeAccessory=>Boolean(item)),counts:Record<string,number>={};for(const item of accessories)counts[item.type]=(counts[item.type]||0)+1;return{configured:true,connected:true,endpoint:status.endpoint,generatedAt,accessories,counts};}catch(reason){return{configured:true,connected:false,endpoint:status.endpoint,generatedAt,accessories:[],counts:{},error:reason instanceof Error?reason.message:String(reason)};}}
